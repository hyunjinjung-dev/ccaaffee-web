...
export default {
  ...
  methods: {
    ...
    async addMenu(column) {
      ...
      if (column == "left") {
        await this.formLeft.push({ name: "", price: "", describe: "" })
        var columnLeft = this.$refs.columnLeft
        columnLeft.scrollTop = columnLeft.scrollHeight
      } else {
        await this.formRight.push({ name: "", price: "", describe: "" })
